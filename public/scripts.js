...
// Integração com IA no Robô Interativo
...
sendButton.addEventListener('click', async () => {
    ...
    try {
        ...
        robotMessage.textContent = data.reply;
...
document.querySelector('#feedback-form').addEventListener('submit', async (e) => {
    ...
    feedbackStatus.textContent = 'Processando...';
    ...
    try {
        ...
        const data = await response.json();
        feedbackStatus.textContent = data.answer || 'Desculpe, não consegui processar sua pergunta.';
    } catch (error) {
        feedbackStatus.textContent = 'Erro ao tentar processar sua solicitação.';
    }
});
